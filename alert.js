...
/* function getName(всегда глагол - что она делает?) (from, text = "текст не добавлен") { //from, text = "текст не добавлен" = локальная переменная)

  тело функции
} 
 let from = "Аня";
// from - параметр; "Аня" - аргумент.
getName ("Аня") - мы показываем аргумент "Аня", принадлежащий параметру from
// "Аня": "текст не добавлен" - вывод на экран конечному пользователю присвоенного аргумента и т.к. не был присовен аргумент text, то выводится "текст не добавлен"
 */

/* function sum(a, b) { //  на входе функция получает аргументы
  return a+b; // return как раз означает немедленный выход из функции(или это возврат значения, так называемого "результата функции". ) 
  и выдача результата, что я напишу в значение - то return и вернет
}
let result = sum(1,2);
alert(result); // 3